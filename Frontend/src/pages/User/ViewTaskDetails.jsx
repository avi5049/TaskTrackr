...
const ViewTaskDetails = () => {
  ...
  const updateTodoChecklist = async (index) => {
    if (!Array.isArray(task?.todoChecklist)) return;
    ...
    try {
      ...
      if (response.status === 200) {
        setTask((prev) => ({
          ...prev,
          todoChecklist: response.data?.task?.todoChecklist ?? todoChecklist,
        }));
      }
    } catch (error) {
              ...
              <div className="mt-2">
                ...
                {task?.todoChecklist?.map((item, index) => (
                  <TodoChecklist
                    ...
                    onChanged={() => updateTodoChecklist(index)}
                  />
                ))}
              </div>
...
const TodoChecklist = ({ text, isChecked, onChanged }) => {
  return (
    <div className="flex items-center gap-3 p-3">
      <input
        ...
        checked={isChecked}
        onChange={onChanged}
        className="w-4 h-4 text-primary bg-gray-100 border-gray-300 outline-none cursor-pointer"
      />
